...
public class PUT extends AbstractHTTPMethod {
	...
	public void execute(Request request, Response response) throws IOException, HTTPException {
		...
		if (isChunked(request)) throw new HTTPException("Chunked posts are not allowed", HTTP.RESPONSE_NOT_IMPLEMENTED);
		...
		catch (BindingAbsentException e)  { throw new HTTPException(e, HTTP.RESPONSE_INTERNAL_SERVER_ERROR, true); }
		catch (BindingPresentException e) { throw new HTTPException(e, HTTP.RESPONSE_INTERNAL_SERVER_ERROR, true); }
		catch (UpdateException e)         { throw new HTTPException(e, HTTP.RESPONSE_INTERNAL_SERVER_ERROR, true); }
		catch (PersistenceException e)    { throw new HTTPException(e, HTTP.RESPONSE_INTERNAL_SERVER_ERROR, true); }
