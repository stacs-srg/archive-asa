 ...
 * @author stuart/al
 */
public class Base16PlaxtonNodeFactory implements INodeFactory {
    
    private IDistanceCalculator dc;
    private ReplacementPolicyFactory rpf;
    
    public Base16PlaxtonNodeFactory(IDistanceCalculator dc, ReplacementPolicyFactory rpf) {
        this.dc = dc;
        this.rpf = rpf;
    }
    
    public IP2PNode makeNode(InetSocketAddress hostAddress, IKey key){
        return new PlaxtonNodeImpl(hostAddress,key,dc,rpf) ;
    }
}
