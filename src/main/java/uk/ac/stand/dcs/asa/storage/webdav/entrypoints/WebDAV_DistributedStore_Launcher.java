...
import uk.ac.stand.dcs.asa.storage.store.impl.distributed.impl.ErrorHandlingGUIDStore;
 ...
 * @author graham
 */
public class WebDAV_DistributedStore_Launcher {
        ...
        if (root_GUID_string != null) {
            ...
            if (portParameter != null) {
 			...
 			try {
				...
				IGUIDStore error_handling_store = new ErrorHandlingGUIDStore(store);
                
		        // Initialise a file system using the store.
		        IFileSystem file_system = new StoreBasedFileSystemFactory(error_handling_store, root_GUID).makeFileSystem();
			...
			catch (FileSystemCreationException e) { Error.hardError("couldn't create file system"); }
			catch (IOException e)                 { Error.hardError("socket error"); }
        }else{
            usage();
        }
	}
