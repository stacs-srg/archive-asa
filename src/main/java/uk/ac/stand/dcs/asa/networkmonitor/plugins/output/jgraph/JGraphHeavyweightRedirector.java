...
public class JGraphHeavyweightRedirector extends MouseInputAdapter {
    ...
    /**
     * Specifies whether this class only redirects to heavyweight cells. Default
     * is false.
     */
    protected boolean heavyweightOnly = false;

    /**
     * Specifies whether this class only redirects to selected cells. Default is
     * true.
     */
    ...
    private void redispatchMouseEvent(final MouseEvent evt) {
        if (evt.getSource() instanceof JGraph) {
            ...
            Object cell = graph.getFirstCellForLocation((int) containerPoint
                    .getX(), (int) containerPoint.getY());
            if (cell != null && (!selectedOnly || graph.isCellSelected(cell))) {
                CellView view = graph.getGraphLayoutCache().getMapping(cell,
                        false);
                if (!heavyweightOnly || view instanceof JGraphHeavyweightView) {
                    ...
                    if (renderer instanceof Container) {
                        isContainer = ((Container) renderer).getComponentCount() > 0;
                    }
                    if (isContainer) {
                        ...
                        if (renderer instanceof JComponent) {
                            ((JComponent) renderer)
                                    .setPreferredSize(new Dimension(
                                            (int) rectBounds.getWidth(),
                                            (int) rectBounds.getHeight()));
                        }
