...
/**
 * NEW_ARCHITECTURE_AL
 * Launcher that starts a WebDav Server, that talks to a FileSystem shim, that talks to a Policy Shim
 * that talks to a local FileStore and a Network FileStore
 *
 * @author al, graham
 */
public class PolicyAwareWebDAVServer_Al {
    ...
    public static void main(String[] args) {
        ...
        if (root_GUID_string != null) {
        	...
        	// No diagnostic by default
    		Diagnostic.setLevel(Diagnostic.NONE);

    		if (CommandLineArgs.getArg(args, "-D") != null) { // switch on diagnostics if required.
