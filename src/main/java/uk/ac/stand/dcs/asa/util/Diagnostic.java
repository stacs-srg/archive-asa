...
public class Diagnostic {
	 ...
	 */
	public static synchronized void initialise(EventBus bus) {
	    
		if (instance.bus == null) instance.bus = bus;
		else Error.hardError("Attempt to reinitialise Diagnostic singleton");
	}
    ...
    private static void outputTrace(String s, Diagnostic level, boolean new_line) {
    	
    	// Synchronise with respect to the Error methods too.
    	synchronized (Error.class) {
        
	        if (level.level_value >= threshold.level_value) outputTrace(s, new_line);
    	}
    }
    
    private static void outputTrace(String s, boolean new_line) {
    	
    	// Synchronise with respect to the Error methods too.
    	synchronized (Error.class) {
	            
            if (instance.bus != null) instance.bus.publishEvent(EventFactory.makeDiagnosticEvent(s));
